...
<head>
	...
	<title>JRuteros - Editar ruta</title>
</head>
<body background="images/background.jpg">
	...
	<div class="col-md-8">
		<% HashMap<String,Ruta> rutas = (HashMap<String,Ruta>)application.getAttribute("rutas");
		 Ruta ruta = rutas.get(request.getParameter("nombre")); %>
		<h2>Editar ruta: <%= ruta.getNombre() %></h2>
		<form action="subirRuta" method="POST" data-toggle="validator" >
			<div class="form-group input-group">
				<input type="text" name="nombre" value="<%= ruta.getNombre() %>" placeholder="Nombre de la ruta" class="form-control" required>
				<span class="input-group-btn" style="width:4px;"></span>
				<input type="number" min="1" name="distancia" value=<%= ruta.getDistancia().toString() %> placeholder="Distancia en KM" class="form-control" required>
			</div>
			<div class="form-group">
				<label>Descripcion</label>
				<textarea type="text" name="descripcion" value=<%= ruta.getDescripcion() %> placeholder="Descripción de la ruta" class=form-control rows="3"></textarea>
			</div>
			<div class="form-group input-group">
				<select class="form-control" value=<%= ruta.getDescripcion() %> name="actividad" required>
